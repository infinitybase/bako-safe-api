...
export async function syncAssets(
...
): Promise<WithId<SchemaFuelAssets>[]> {
  const uniqueAssetIds = [...new Set(deposits.map((d) => d.assetId))];
  let assets = await assets_collection
    .find({ _id: { $in: uniqueAssetIds } })
    ?.toArray();
  const assetIds = assets.map((a) => a._id);
  const missingAssets = [
    ...new Set(
      deposits
        .filter((d) => !assetIds.includes(d.assetId))
        .map((d) => d.assetId)
    ),
  ];
  ...
  for await (const missAsset of missingAssets) {
    const asset = await fetch(
      `https://mainnet-explorer.fuel.network/assets/${missAsset}`
    );
